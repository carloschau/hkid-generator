    ...
    <ThemeProvider theme={theme}>
      ...
      <Container sx={{display:'flex', flexDirection:'column', alignItems:'center', marginTop: '30px' }}>
        ...
        <Box sx={{cursor: 'pointer', display:'flex', flexWrap: 'wrap'}} onClick={() =>{
          ...
          {hkidList.map((hkid, i) => <Typography key={hkid+i} sx={{ fontSize: '2.188rem', fontFamily: 'Source Sans Pro, sans-serif', letterSpacing: '6px', fontWeight: '700', margin:'0 15px'}} color='primary' >{hkid}</Typography>)}
        </Box>
